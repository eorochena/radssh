    ...
    def authenticate(self, T, sshconfig={}):
        ...
        for auth_type in preferred_auth_types:
            ...
            if auth_type == 'publickey' and sshconfig.get('pubkeyauthentication', 'yes') == 'yes':
                ...
                if auth_type in ('password', 'keyboard-interactive') and (
                        sshconfig.get('passwordauthentication', 'yes') == 'yes' or
                        sshconfig.get('kbdinteractiveauthentication', 'yes') == 'yes'):
                    auth_success = self.try_auth(T, self.passwords, True, auth_user)
                ...
                if as_password:
                    ...
                    while True:
                        if 'password' not in T.auth_password(auth_user, str(key)):
                            break
                    if T.auth_handler.get_banner() and not T.save_banner:
                        T.save_banner = T.auth_handler.get_banner()
                else:
